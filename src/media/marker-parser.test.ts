...
describe('parseMediaReferences', () => {
  ...
  it('returns a single ref for a valid image attachment', () => {
    ...
    fs.writeFileSync(
      path.join(tmp, 'groups', 'main', 'attachments', file),
      Buffer.alloc(1),
    );
    const r = row('1', [
      {
        kind: 'image',
        handlerName: 'image',
        workspacePath: `attachments/${file}`,
        mimetype: 'image/jpeg',
      },
    ]);
    const refs = parseMediaReferences([r], 'main', tmp);
    expect(refs).toEqual([
      { relativePath: `attachments/${file}`, mediaType: 'image/jpeg' },
    ]);
  });
    ...
    const r = row('1', [
      {
        kind: 'image',
        handlerName: 'image',
        workspacePath: 'attachments/x.jpg',
        mimetype: 'image/jpeg',
        error: 'fail',
      },
    ]);
    ...
    const r = row('1', [
      {
        kind: 'image',
        handlerName: 'image',
        workspacePath: 'attachments/missing.jpg',
        mimetype: 'image/jpeg',
      },
    ]);
    expect(parseMediaReferences([r], 'main', tmp)).toEqual([]);
    ...
    const r = row('1', [
      {
        kind: 'image',
        handlerName: 'image',
        workspacePath: 'attachments/../../other/x.jpg',
        mimetype: 'image/jpeg',
      },
    ]);
    expect(parseMediaReferences([r], 'main', tmp)).toEqual([]);
  ...
  it('returns refs in timestamp order across multiple rows', () => {
    fs.writeFileSync(
      path.join(tmp, 'groups', 'main', 'attachments', 'a.jpg'),
      Buffer.alloc(1),
    );
    fs.writeFileSync(
      path.join(tmp, 'groups', 'main', 'attachments', 'b.jpg'),
      Buffer.alloc(1),
    );
    const r1: Row = {
      id: '1',
      ...
      attachments: [
        {
          kind: 'image',
          handlerName: 'image',
          workspacePath: 'attachments/a.jpg',
          mimetype: 'image/jpeg',
        },
      ],
    };
      ...
      attachments: [
        {
          kind: 'image',
          handlerName: 'image',
          workspacePath: 'attachments/b.jpg',
          mimetype: 'image/jpeg',
        },
      ],
    };
