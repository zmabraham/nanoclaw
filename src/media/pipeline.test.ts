...
function handler(name: string, kind: MediaRef['kind'], priority: number, fn: (ref: MediaRef, ctx: HandlerContext, signal: AbortSignal) => Promise<ProcessedMedia>): MediaHandler {
...
describe('MediaPipeline', () => {
  ...
  it('highest priority handler wins', async () => {
    registerMediaHandler(handler('low', 'image', 100, async () => ({
      textRepresentation: 'low', handlerName: 'low', durationMs: 1,
    })));
    registerMediaHandler(handler('high', 'image', 200, async () => ({
      textRepresentation: 'high', handlerName: 'high', durationMs: 1,
    })));
    const bundle = await pipelineWithRoot().process([baseRef()], 'main', 'c', 'm');
    expect(bundle.entries[0].handlerName).toBe('high');
  });

  it('size cap rejects before dispatch', async () => {
    registerMediaHandler(handler('x', 'image', 100, async () => ({
      textRepresentation: 'x', handlerName: 'x', durationMs: 1,
    })));
    const huge = baseRef({ sizeBytes: 100 * 1024 * 1024 });
    const bundle = await new MediaPipeline({ projectRoot: tmp, maxBytes: 25 * 1024 * 1024 }).process([huge], 'main', 'c', 'm');
  ...
  it('per-handler 60s timeout fires with fallback; abandoned promise gets .catch', async () => {
    ...
    const probe = () => { unhandledSeen = true; };
    process.on('unhandledRejection', probe);
    try {
      registerMediaHandler(handler('slow', 'image', 100, async (_r, _c, signal) => {
        // Ignores signal; simulates a rogue handler that rejects LATE
        return new Promise((_, reject) => setTimeout(() => reject(new Error('late')), 300));
      }));
      const bundle = await new MediaPipeline({ projectRoot: tmp, handlerTimeoutMs: 50 }).process([baseRef()], 'main', 'c', 'm');
  ...
  it('writes workspace files (single-writer invariant)', async () => {
    registerMediaHandler(handler('img', 'image', 100, async () => ({
      textRepresentation: '[Image: attachments/f.jpg]', handlerName: 'img', durationMs: 1,
      workspaceFile: { relativePath: 'attachments/f.jpg', bytes: Buffer.from('abc') },
    })));
    const bundle = await pipelineWithRoot().process([baseRef()], 'main', 'c', 'm');
    const file = path.join(tmp, 'groups', 'main', 'attachments', 'f.jpg');
    expect(fs.existsSync(file)).toBe(true);
    expect(fs.readFileSync(file).toString()).toBe('abc');
    expect(bundle.attachmentSummaries[0].workspacePath).toBe('attachments/f.jpg');
  });
  ...
  it('records output mimetype from handler when transcoding (e.g. PNG input → JPEG output)', async () => {
    ...
    const bundle = await pipelineWithRoot().process([pngInput], 'main', 'c', 'm');
  ...
  it('falls back to input mimetype when handler does not set outputMimetype', async () => {
    registerMediaHandler(handler('pdf', 'document', 100, async () => ({
      textRepresentation: '[PDF: attachments/d.pdf]',
      handlerName: 'pdf',
      durationMs: 1,
      workspaceFile: { relativePath: 'attachments/d.pdf', bytes: Buffer.from('pdf-bytes') },
    })));
    const pdfInput = baseRef({ kind: 'document', mimetype: 'application/pdf' });
    const bundle = await pipelineWithRoot().process([pdfInput], 'main', 'c', 'm');
  ...
  it('aggregates text representations with single-space join, trims', async () => {
    registerMediaHandler(handler('img', 'image', 100, async () => ({
      textRepresentation: '[Image: x]', handlerName: 'img', durationMs: 1,
    })));
    registerMediaHandler(handler('vid', 'video', 100, async () => ({
      textRepresentation: '[Video: y]', handlerName: 'vid', durationMs: 1,
    })));
  ...
  it('concurrency cap gates parallel bundles', async () => {
    ...
    const ref = baseRef();
    await Promise.all(Array.from({ length: 6 }, (_, i) => pipeline.process([ref], 'main', 'c', `m${i}`)));
